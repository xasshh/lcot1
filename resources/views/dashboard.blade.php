        ...
        <div class="p-4 sm:p-6 lg:p-8 max-w-screen-xl mx-auto">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm db-card mb-6 overflow-hidden">
                <div class="px-5 sm:px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-4">
                    <div>
                        <h3 class="text-sm font-bold text-slate-800">Enrolled Courses</h3>
                        <p class="text-xs text-slate-400 mt-0.5">Current semester registration</p>
                    </div>
                    <span class="flex-shrink-0 text-xs font-semibold bg-red-50 text-red-700 border border-red-200 px-2.5 py-1 rounded-full">
                        ...
                        <tbody>
                            @forelse(auth()->user()->courses as $course)
                                <tr>
                                    ...
                                    <td>
                                        <span style="display:inline-flex;align-items:center;justify-content:center;width:1.75rem;height:1.75rem;border-radius:50%;background:#fef2f2;color:#b91c1c;font-size:0.78rem;font-weight:700;border:1px solid #fecaca;">
                                            {{ $course->unit }}
                                        </span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" style="text-align:center;padding:2.5rem 1rem;color:#94a3b8;font-size:0.875rem;">
            ...
            {{-- ── Timetables ── --}}
            @php
                if (!function_exists('renderTimetable')) {
                    function renderTimetable($timetableText) {
                        if (!$timetableText) {
                            return '<p style="color:#94a3b8;font-size:0.875rem;padding:1.5rem 1rem;">No timetable available.</p>';
                        }
                        $rows = explode("\n", trim($timetableText));
                        ...
                        foreach ($rows as $i => $row) {
                            ...
                            $html .= '<tr>';
                            foreach ($cols as $col) {
                                $safe = htmlspecialchars($col, ENT_QUOTES, 'UTF-8');
                                if ($i === 0) {
                                    $html .= "<th style=\"background:#1e293b;color:#f1f5f9;padding:9px 14px;text-align:left;font-size:0.68rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;white-space:nowrap;\">{$safe}</th>";
                                } else {
                                    $html .= "<td style=\"padding:9px 14px;color:#475569;border-bottom:1px solid #f1f5f9;font-size:0.84rem;vertical-align:middle;\">{$safe}</td>";
                                }
                            }
                            $html .= '</tr>';
                        }
