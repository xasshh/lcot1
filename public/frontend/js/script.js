...
// Program Center to Program Taken mapping
...
// Function to update matric number prefix
function updateMatricPrefix() {
    const programCenterSelect = document.getElementById('programCenter');
    const programTakenSelect = document.getElementById('programTaken');
    const yearAdmittedSelect = document.getElementById('yearAdmitted');
    const matricPrefix = document.getElementById('matricPrefix');
    
    if (!matricPrefix) return;
    
    let prefix = '';
    
    // Get center prefix (first two letters)
    if (programCenterSelect && programCenterSelect.value) {
        const selectedOption = programCenterSelect.options[programCenterSelect.selectedIndex];
        const centerText = selectedOption.textContent.replace(' Center', ''); // Remove " Center" from text
        const centerPrefix = centerText.substring(0, 2).toUpperCase();
        prefix += centerPrefix + '/';
    } else {
        prefix += '--/';
    }
    ...
    // Get program taken
    if (programTakenSelect && programTakenSelect.value) {
        const selectedProgramOption = programTakenSelect.options[programTakenSelect.selectedIndex];
        const programText = selectedProgramOption.textContent;
        prefix += programText + '/';
    } else {
        prefix += '--/';
    }
    ...
    // Get year (full year)
    if (yearAdmittedSelect && yearAdmittedSelect.value) {
        const year = yearAdmittedSelect.value;
        prefix += year + '/';
    } else {
        prefix += '----/';
    }
    
    matricPrefix.textContent = prefix;
}

// Handle Program Center selection change
document.addEventListener('DOMContentLoaded', function() {
    const programCenterSelect = document.getElementById('programCenter');
    const programTakenSelect = document.getElementById('programTaken');
    const yearAdmittedSelect = document.getElementById('yearAdmitted');
    
    if (programCenterSelect && programTakenSelect) {
        programCenterSelect.addEventListener('change', function() {
            const selectedCenter = this.value;
            
            // Clear previous options
            programTakenSelect.innerHTML = '';
            
            if (selectedCenter === '') {
                // No center selected
                programTakenSelect.disabled = true;
                programTakenSelect.innerHTML = '<option value="">Select Program Center first</option>';
            } else {
                // Enable dropdown and populate with available programs
                programTakenSelect.disabled = false;
                programTakenSelect.innerHTML = '<option value="">Select Program Taken</option>';
                
                const availablePrograms = programCenterMapping[selectedCenter] || [];
                availablePrograms.forEach(program => {
                    const option = document.createElement('option');
                    option.value = program.toLowerCase();
                    option.textContent = program;
                    programTakenSelect.appendChild(option);
                });
            }
            
            // Update matric prefix
            updateMatricPrefix();
        });
        
        // Update matric prefix when program taken changes
        programTakenSelect.addEventListener('change', function() {
            updateMatricPrefix();
        });
    }
    
    // Update matric prefix when year admitted changes
    if (yearAdmittedSelect) {
        yearAdmittedSelect.addEventListener('change', function() {
            updateMatricPrefix();
        });
    }
